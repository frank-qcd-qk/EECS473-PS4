...
void moveRobotTo(float x, float y, float z, int resolution, int motionTime){
    ...
    // * action    
    flange_origin << x, y, z; //manually prescribed flange pose; in the  future, compute this based  on perception
    ...
    ROS_INFO("g_q_vec_arm Passed!");
    optimal_path.clear();
    ROS_INFO("pass optimal_path clear");
    ...
    ROS_INFO("Pass pCartTrajPlanner");
...
int main(int argc, char** argv) {
    ...
    traj_publisher = nh.advertise<trajectory_msgs::JointTrajectory>("joint_path_command", 1);
    ...
    ROS_INFO("Information Pass all successful!");
    ...
    while (t<6.28){
        ...
        float x = (13 * cos(t) - 5*cos(2*t) - 2 * cos(3*t)- cos(4*t))/100 + 0.35;
        t = t + 0.1;
